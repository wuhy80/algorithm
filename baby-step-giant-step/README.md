...
**Baby-Step Giant-Step 离散对数** 属于“整数结构与模运算”这一类问题。先不要急着背实现：它的核心任务是：求解模意义下 a^x≡b 的离散对数。
...
把指数拆成约 √m 的婴儿步与巨人步，并用哈希表寻找相遇值。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **Baby-Step Giant-Step 离散对数** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 明确整数范围、符号与模数前提
2. 利用整除或同余性质缩小问题。在本算法中，对应演示动作是：把指数拆成约 √m 的婴儿步与巨人步，并用哈希表寻找相遇值。
3. 重复执行可证明等价的递推
4. 处理溢出、负余数和不存在解的情况
...
3. 对照状态栏观察“把指数拆成约 √m 的婴儿步与巨人步，并用哈希表寻找相遇值。”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[快速模幂 Modular Exponentiation](https://github.com/wuhy80/algorithm/tree/main/modular-exponentiation/)、[扩展欧几里得 Extended Euclidean](https://github.com/wuhy80/algorithm/tree/main/extended-euclidean/)
- 直接后续：暂无强制关联项。
- 同类比较：[埃拉托斯特尼筛法](https://github.com/wuhy80/algorithm/tree/main/sieve-of-eratosthenes/)、[分段筛 Segmented Sieve](https://github.com/wuhy80/algorithm/tree/main/segmented-sieve/)、[欧几里得算法 Euclidean](https://github.com/wuhy80/algorithm/tree/main/euclidean-algorithm/)、[欧拉函数筛 Totient](https://github.com/wuhy80/algorithm/tree/main/euler-totient/)

学习顺序建议是：先用先修算法理解基础状态，再比较同类算法在“前提、维护信息、复杂度、是否可恢复答案”上的差异，最后进入把本算法作为组件的后续主题。

## 自测问题

- 不看代码，你能否用一句话说清 **Baby-Step Giant-Step 离散对数** 在每一步维护的状态？
