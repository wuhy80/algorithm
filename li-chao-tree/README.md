...
**Li Chao Tree** 属于“区间分解与可合并摘要”这一类问题。先不要急着背实现：它的核心任务是：处理直线斜率和查询顺序均不单调的动态最值问题。
...
在线维护直线集合并查询指定横坐标的最优值。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
2. 构建标准区间或块的摘要。在本算法中，对应演示动作是：在线维护直线集合并查询指定横坐标的最优值
3. 查询时选择完全覆盖的块并合并
4. 更新时只修改受影响路径并重算祖先
