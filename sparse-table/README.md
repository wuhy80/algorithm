...
> 2 的幂次区间预处理以及双区间块常数时间查询

## 先抓住一句话

**稀疏表 Sparse Table** 属于“区间分解与可合并摘要”这一类问题。先不要急着背实现：它的核心任务是：2 的幂次区间预处理以及双区间块常数时间查询。在同一批数据上高效回答多次区间查询或更新，避免每个操作重新扫描完整范围。
...
2 的幂次区间预处理以及双区间块常数时间查询。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **稀疏表 Sparse Table** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义区间摘要以及 combine 是否满足结合律
2. 构建标准区间或块的摘要。在本算法中，对应演示动作是：2 的幂次区间预处理以及双区间块常数时间查询
3. 查询时选择完全覆盖的块并合并
4. 更新时只修改受影响路径并重算祖先
...
3. 对照状态栏观察“2 的幂次区间预处理以及双区间块常数时间查询”是否正在发生，并解释为什么没有选择其他候选。
4. 到达终态后只改一个边界条件，例如空输入、重复值、极端顺序、断开的图或最小规模，再重新运行。
5. 最后尝试不看动画，只根据伪代码复现同一过程；能独立写出状态变化才算真正掌握。

## 复杂度怎么分析

**结论：预处理 O(n log n)，幂等区间查询 O(1)，空间 O(n log n)。**
...
- 直接后续：[不相交稀疏表 Disjoint Sparse Table](https://github.com/wuhy80/algorithm/tree/main/disjoint-sparse-table/)
...
- 不看代码，你能否用一句话说清 **稀疏表 Sparse Table** 在每一步维护的状态？
...
- [打开演示](https://wuhy80.github.io/algorithm/sparse-table/)
- [查看源码](https://github.com/wuhy80/algorithm/tree/main/sparse-table/)
