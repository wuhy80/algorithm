...
**0-1 BFS** 属于“距离估计与边松弛”这一类问题。先不要急着背实现：它的核心任务是：在线性时间内求只有 0/1 边权图的单源最短路。
...
双端队列按边权把节点加入队首或队尾，逐边展示距离松弛。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **0-1 BFS** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 把起点距离设为 0，其余设为无穷
2. 按算法规则选择下一批待处理节点。在本算法中，对应演示动作是：双端队列按边权把节点加入队首或队尾，逐边展示距离松弛。
...
3. 对照状态栏观察“双端队列按边权把节点加入队首或队尾，逐边展示距离松弛。”是否正在发生，并解释为什么没有选择其他候选。
