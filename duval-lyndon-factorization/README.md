...
**Duval / Lyndon 分解** 属于“后缀、自动机与全文索引”这一类问题。先不要急着背实现：它的核心任务是：解决最小循环表示、重复结构分析和组合字符串问题。
...
线性构造字符串唯一的非增 Lyndon 因子序列。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
2. 按字符或倍增长度增量构建结构。在本算法中，对应演示动作是：线性构造字符串唯一的非增 Lyndon 因子序列
3. 维护链接、排名或区间边界
4. 把查询转成路径、区间或公共前缀操作
