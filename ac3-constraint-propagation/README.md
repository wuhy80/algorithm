...
**AC-3 约束传播** 属于“约束传播与搜索剪枝”这一类问题。先不要急着背实现：它的核心任务是：在搜索前缩减数独、图着色、排程等约束满足问题的候选空间。
...
反复修订变量取值域直到所有二元约束达到弧一致。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **AC-3 约束传播** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义变量、取值域和约束
2. 传播单位条件或弧一致性。在本算法中，对应演示动作是：反复修订变量取值域直到所有二元约束达到弧一致
3. 选择仍未决定的变量和候选值
4. 冲突时回滚，完整赋值时输出解
...
3. 对照状态栏观察“反复修订变量取值域直到所有二元约束达到弧一致”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[图着色回溯 Graph Coloring](https://github.com/wuhy80/algorithm/tree/main/graph-coloring/)
- 直接后续：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **AC-3 约束传播** 在每一步维护的状态？
