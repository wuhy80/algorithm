...
> 通过父子边构造任意分支数量的树并统计指定子树

## 先抓住一句话

**多叉树 N-ary Tree** 属于“递归子树与层级关系”这一类问题。先不要急着背实现：它的核心任务是：表达文件目录、组织架构、DOM 和分类体系等一个节点可拥有多个孩子的层级数据。
...
通过父子边构造任意分支数量的树并统计指定子树。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **多叉树 N-ary Tree** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义节点、孩子和递归函数的返回值
2. 处理空节点或叶子节点基例。在本算法中，对应演示动作是：通过父子边构造任意分支数量的树并统计指定子树
3. 递归取得各子树结果
4. 在当前节点合并并返回
...
3. 对照状态栏观察“通过父子边构造任意分支数量的树并统计指定子树”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[二叉树基础 Binary Tree Basics](https://github.com/wuhy80/algorithm/tree/main/binary-tree-basics/)
- 直接后续：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **多叉树 N-ary Tree** 在每一步维护的状态？
...
- [打开演示](https://wuhy80.github.io/algorithm/n-ary-tree/)
- [查看源码](https://github.com/wuhy80/algorithm/tree/main/n-ary-tree/)
