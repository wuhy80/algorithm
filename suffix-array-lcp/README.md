...
> 倍增排序全部后缀并计算相邻后缀最长公共前缀

## 先抓住一句话

**后缀数组与 LCP** 属于“后缀、自动机与全文索引”这一类问题。先不要急着背实现：它的核心任务是：倍增排序全部后缀并计算相邻后缀最长公共前缀。为文本建立后缀或自动机索引，使大量子串、重复和词典序查询能够共享结构。
...
倍增排序全部后缀并计算相邻后缀最长公共前缀。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **后缀数组与 LCP** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 确定索引表示的是前缀、后缀还是等价子串集合
2. 按字符或倍增长度增量构建结构。在本算法中，对应演示动作是：倍增排序全部后缀并计算相邻后缀最长公共前缀
3. 维护链接、排名或区间边界
4. 把查询转成路径、区间或公共前缀操作
...
3. 对照状态栏观察“倍增排序全部后缀并计算相邻后缀最长公共前缀”是否正在发生，并解释为什么没有选择其他候选。
4. 到达终态后只改一个边界条件，例如空输入、重复值、极端顺序、断开的图或最小规模，再重新运行。
5. 最后尝试不看动画，只根据伪代码复现同一过程；能独立写出状态变化才算真正掌握。

## 复杂度怎么分析

**结论：倍增构造通常 O(n log n)，Kasai 求 LCP 为 O(n)，空间 O(n)。**
...
- 直接后续：[后缀树 Suffix Tree](https://github.com/wuhy80/algorithm/tree/main/suffix-tree/)、[Burrows-Wheeler 变换 BWT](https://github.com/wuhy80/algorithm/tree/main/burrows-wheeler-transform/)、[FM-index](https://github.com/wuhy80/algorithm/tree/main/fm-index/)
- 同类比较：[后缀自动机 Suffix Automaton](https://github.com/wuhy80/algorithm/tree/main/suffix-automaton/)、[Aho-Corasick 多模式匹配](https://github.com/wuhy80/algorithm/tree/main/aho-corasick/)、[Booth 最小表示法](https://github.com/wuhy80/algorithm/tree/main/booth-minimum-rotation/)、[Duval / Lyndon 分解](https://github.com/wuhy80/algorithm/tree/main/duval-lyndon-factorization/)

学习顺序建议是：先用先修算法理解基础状态，再比较同类算法在“前提、维护信息、复杂度、是否可恢复答案”上的差异，最后进入把本算法作为组件的后续主题。

## 自测问题

- 不看代码，你能否用一句话说清 **后缀数组与 LCP** 在每一步维护的状态？
...
- [打开演示](https://wuhy80.github.io/algorithm/suffix-array-lcp/)
- [查看源码](https://github.com/wuhy80/algorithm/tree/main/suffix-array-lcp/)
