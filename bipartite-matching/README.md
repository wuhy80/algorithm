...
**二分图最大匹配** 属于“交替路与匹配增广”这一类问题。先不要急着背实现：它的核心任务是：交替增广路径搜索与已有匹配重新安排。选择互不共享端点的边完成配对或指派，并通过增广重新安排局部选择。
...
交替增广路径搜索与已有匹配重新安排。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **二分图最大匹配** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 维护每个节点当前匹配对象
2. 从未匹配节点搜索交替路。在本算法中，对应演示动作是：交替增广路径搜索与已有匹配重新安排
3. 找到未匹配终点后回溯翻转边状态
4. 重复直到不存在增广路
...
- 直接后续：[匈牙利算法 Hungarian](https://github.com/wuhy80/algorithm/tree/main/hungarian-algorithm/)、[Blossom 一般图匹配](https://github.com/wuhy80/algorithm/tree/main/blossom-matching/)、[Hopcroft-Karp 匹配](https://github.com/wuhy80/algorithm/tree/main/hopcroft-karp/)、[稳定婚姻匹配 Gale-Shapley](https://github.com/wuhy80/algorithm/tree/main/stable-marriage/)
- 同类比较：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **二分图最大匹配** 在每一步维护的状态？
...
- [打开演示](https://wuhy80.github.io/algorithm/bipartite-matching/)
- [查看源码](https://github.com/wuhy80/algorithm/tree/main/bipartite-matching/)
