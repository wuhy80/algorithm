...
**二叉树后序遍历 Postorder Traversal** 属于“递归子树与层级关系”这一类问题。先不要急着背实现：它的核心任务是：先获得两个子树结果再处理根，适合释放树、计算子树值和表达式求值。
...
按照左、右、根的顺序演示子树完成后再处理父节点。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 定义节点、孩子和递归函数的返回值
2. 处理空节点或叶子节点基例。在本算法中，对应演示动作是：按照左、右、根的顺序演示子树完成后再处理父节点
3. 递归取得各子树结果
4. 在当前节点合并并返回
