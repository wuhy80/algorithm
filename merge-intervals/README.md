...
**合并区间 Merge Intervals** 属于“单调指针与一次扫描”这一类问题。先不要急着背实现：它的核心任务是：把相互重叠的时间段或数值范围合并为最少的不相交区间。
...
排序区间后逐个判断重叠并扩展当前覆盖范围。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **合并区间 Merge Intervals** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义左右指针和它们围成区域的含义
2. 扩展一个边界直到条件变化。在本算法中，对应演示动作是：排序区间后逐个判断重叠并扩展当前覆盖范围
3. 收缩另一个边界恢复不变量
4. 在每次有效状态下更新答案
