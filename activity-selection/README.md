...
**活动选择 Activity Selection** 属于“局部选择与交换论证”这一类问题。先不要急着背实现：它的核心任务是：从一组起止时间已知的活动中选择数量最多的互不重叠活动。
...
按结束时间排序并选择互不冲突的活动，动态展示候选判断与已选集合。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **活动选择 Activity Selection** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 找出可比较的局部选择标准
2. 按该标准排序或维护最优候选。在本算法中，对应演示动作是：按结束时间排序并选择互不冲突的活动，动态展示候选判断与已选集合。
3. 选择不破坏可行性的候选
...
3. 对照状态栏观察“按结束时间排序并选择互不冲突的活动，动态展示候选判断与已选集合。”是否正在发生，并解释为什么没有选择其他候选。
...
- 直接后续：[带权区间调度](https://github.com/wuhy80/algorithm/tree/main/weighted-interval-scheduling/)、[分数背包 Fractional Knapsack](https://github.com/wuhy80/algorithm/tree/main/fractional-knapsack/)、[区间覆盖 Interval Covering](https://github.com/wuhy80/algorithm/tree/main/interval-covering/)、[任务序列调度 Job Sequencing](https://github.com/wuhy80/algorithm/tree/main/job-sequencing/)
- 同类比较：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **活动选择 Activity Selection** 在每一步维护的状态？
