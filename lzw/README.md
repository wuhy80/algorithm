...
**LZW 压缩** 属于“重复、概率与可逆表示”这一类问题。先不要急着背实现：它的核心任务是：短语词典动态扩展和编码编号依次输出。发现频率偏斜或重复结构，生成可唯一解码且通常更短的数据表示。
...
短语词典动态扩展和编码编号依次输出。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **LZW 压缩** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 分析符号频率或可重复片段
2. 选择码字、区间或字典引用。在本算法中，对应演示动作是：短语词典动态扩展和编码编号依次输出
3. 按顺序输出可唯一解析的标记
4. 使用同一规则解码并逐项校验原文
...
- 不看代码，你能否用一句话说清 **LZW 压缩** 在每一步维护的状态？
...
- [打开演示](https://wuhy80.github.io/algorithm/lzw/)
- [查看源码](https://github.com/wuhy80/algorithm/tree/main/lzw/)
