...
**高斯消元 Gaussian Elimination** 属于“换一个基底看计算”这一类问题。先不要急着背实现：它的核心任务是：求解线性方程组、计算秩并判断解的存在性。
...
通过部分选主元、行归一化和逐列消元得到行最简形与唯一解。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 把输入补齐到算法要求的长度或维度
2. 按分治结构执行正变换。在本算法中，对应演示动作是：通过部分选主元、行归一化和逐列消元得到行最简形与唯一解。
3. 在变换域逐点完成目标运算
4. 执行逆变换并处理舍入 / 取模
...
- 直接后续：暂无强制关联项。
- 同类比较：[傅里叶旋轮 Fourier Epicycles](https://github.com/wuhy80/algorithm/tree/main/fourier-epicycles/)、[快速傅里叶变换 FFT](https://github.com/wuhy80/algorithm/tree/main/fft/)、[数论变换 NTT](https://github.com/wuhy80/algorithm/tree/main/ntt/)
