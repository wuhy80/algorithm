...
**Euler Tour Tree** 属于“递归子树与层级关系”这一类问题。先不要急着背实现：它的核心任务是：在动态森林中支持子树剪切、重新连接、连通性和聚合查询。
...
把树的进入/退出事件存入隐式 Treap，通过序列切分合并移动整棵子树。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 定义节点、孩子和递归函数的返回值
2. 处理空节点或叶子节点基例。在本算法中，对应演示动作是：把树的进入/退出事件存入隐式 Treap，通过序列切分合并移动整棵子树。
3. 递归取得各子树结果
4. 在当前节点合并并返回
