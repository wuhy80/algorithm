...
**欧拉路径 Eulerian Path** 属于“图搜索与访问状态”这一类问题。先不要急着背实现：它的核心任务是：Hierholzer 栈沿未使用边深入并在回退时构造完整路径。系统访问图中的节点与边，回答可达性、层次、遍历顺序或基础结构判定。
...
Hierholzer 栈沿未使用边深入并在回退时构造完整路径。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 由边构建邻接关系并确定有向性
2. 初始化起点、visited 与栈 / 队列。在本算法中，对应演示动作是：Hierholzer 栈沿未使用边深入并在回退时构造完整路径
3. 取出节点并检查相邻边
4. 发现新节点时记录来源、层数或颜色
