...
**八叉树 Octree** 属于“空间分区与候选剪枝”这一类问题。先不要急着背实现：它的核心任务是：索引三维点云、体素与空间对象，并剪枝范围和邻近查询。
...
容量超限的三维立方体沿三个坐标轴同时二分，递归形成八个子体素。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **八叉树 Octree** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 选择划分轴、象限或包围盒策略
2. 递归分配对象并计算节点边界。在本算法中，对应演示动作是：容量超限的三维立方体沿三个坐标轴同时二分，递归形成八个子体素。
3. 查询时先测试节点边界
...
3. 对照状态栏观察“容量超限的三维立方体沿三个坐标轴同时二分，递归形成八个子体素。”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[四叉树 Quadtree](https://github.com/wuhy80/algorithm/tree/main/quadtree/)
- 直接后续：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **八叉树 Octree** 在每一步维护的状态？
