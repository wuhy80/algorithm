...
**位集与位图 Bitset / Bitmap** 属于“键、哈希值与冲突”这一类问题。先不要急着背实现：它的核心任务是：紧凑保存大规模布尔集合，并快速完成成员测试、交并差和状态压缩。
...
用机器字的单个位表达布尔状态并执行批量位运算。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 规范化键并计算哈希值
2. 映射到桶或初始槽位。在本算法中，对应演示动作是：用机器字的单个位表达布尔状态并执行批量位运算
3. 按链式或开放寻址规则解决冲突
4. 在负载过高时扩容并重新散列
...
- 直接后续：暂无强制关联项。
- 同类比较：[哈希表 Hash Table](https://github.com/wuhy80/algorithm/tree/main/hash-table/)、[Bloom Filter](https://github.com/wuhy80/algorithm/tree/main/bloom-filter/)、[LRU 缓存](https://github.com/wuhy80/algorithm/tree/main/lru-cache/)、[集合与映射 ADT Set / Map](https://github.com/wuhy80/algorithm/tree/main/set-map-adt/)
