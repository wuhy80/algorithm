...
**后缀自动机 Suffix Automaton** 属于“后缀、自动机与全文索引”这一类问题。先不要急着背实现：它的核心任务是：以线性规模结构表示一个字符串的全部子串并回答子串问题。
...
逐字符扩展状态、后缀链接与转移，必要时创建克隆状态。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **后缀自动机 Suffix Automaton** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 确定索引表示的是前缀、后缀还是等价子串集合
2. 按字符或倍增长度增量构建结构。在本算法中，对应演示动作是：逐字符扩展状态、后缀链接与转移，必要时创建克隆状态。
3. 维护链接、排名或区间边界
4. 把查询转成路径、区间或公共前缀操作
...
3. 对照状态栏观察“逐字符扩展状态、后缀链接与转移，必要时创建克隆状态。”是否正在发生，并解释为什么没有选择其他候选。
