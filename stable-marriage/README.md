...
**稳定婚姻匹配 Gale-Shapley** 属于“交替路与匹配增广”这一类问题。先不要急着背实现：它的核心任务是：为两组带偏好顺序的参与者寻找不存在阻塞对的稳定匹配。
...
自由提议者依偏好提议，接收者始终保留当前更偏好的匹配。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 维护每个节点当前匹配对象
2. 从未匹配节点搜索交替路。在本算法中，对应演示动作是：自由提议者依偏好提议，接收者始终保留当前更偏好的匹配。
3. 找到未匹配终点后回溯翻转边状态
4. 重复直到不存在增广路
...
- 直接后续：暂无强制关联项。
- 同类比较：[匈牙利算法 Hungarian](https://github.com/wuhy80/algorithm/tree/main/hungarian-algorithm/)、[Blossom 一般图匹配](https://github.com/wuhy80/algorithm/tree/main/blossom-matching/)、[Hopcroft-Karp 匹配](https://github.com/wuhy80/algorithm/tree/main/hopcroft-karp/)
