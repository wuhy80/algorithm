...
**差分数组 Difference Array** 属于“区间分解与可合并摘要”这一类问题。先不要急着背实现：它的核心任务是：高效处理大量离线区间加法，再一次性生成更新后的数组。
...
区间更新只修改两个差分边界，最后通过前缀累加恢复完整数组。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
2. 构建标准区间或块的摘要。在本算法中，对应演示动作是：区间更新只修改两个差分边界，最后通过前缀累加恢复完整数组。
3. 查询时选择完全覆盖的块并合并
4. 更新时只修改受影响路径并重算祖先
...
- 直接后续：暂无强制关联项。
- 同类比较：[可持久化线段树](https://github.com/wuhy80/algorithm/tree/main/persistent-segment-tree/)、[懒标记线段树](https://github.com/wuhy80/algorithm/tree/main/lazy-segment-tree/)、[区间树 Interval Tree](https://github.com/wuhy80/algorithm/tree/main/interval-tree/)、[树状数组 Fenwick Tree](https://github.com/wuhy80/algorithm/tree/main/fenwick-tree/)
