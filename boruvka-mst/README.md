...
**Borůvka 最小生成树** 属于“安全边与连通分量”这一类问题。先不要急着背实现：它的核心任务是：为连通带权无向图寻找总权重最小的生成树。
...
所有连通分量同时选择最轻出边，批量合并并构建最小生成树。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **Borůvka 最小生成树** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 初始化每个节点为独立分量或选择起点
2. 按权重或跨割候选顺序检查边。在本算法中，对应演示动作是：所有连通分量同时选择最轻出边，批量合并并构建最小生成树。
3. 只接受连接不同分量的安全边
...
3. 对照状态栏观察“所有连通分量同时选择最轻出边，批量合并并构建最小生成树。”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[Kruskal 最小生成树](https://github.com/wuhy80/algorithm/tree/main/kruskal-mst/)、[并查集 Union-Find](https://github.com/wuhy80/algorithm/tree/main/union-find/)
- 直接后续：暂无强制关联项。
- 同类比较：[Chu-Liu/Edmonds 有向最小生成树](https://github.com/wuhy80/algorithm/tree/main/chu-liu-edmonds/)、[Prim 最小生成树](https://github.com/wuhy80/algorithm/tree/main/prim-mst/)

学习顺序建议是：先用先修算法理解基础状态，再比较同类算法在“前提、维护信息、复杂度、是否可恢复答案”上的差异，最后进入把本算法作为组件的后续主题。

## 自测问题

- 不看代码，你能否用一句话说清 **Borůvka 最小生成树** 在每一步维护的状态？
