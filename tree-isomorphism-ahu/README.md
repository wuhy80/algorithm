...
**AHU 树同构** 属于“递归子树与层级关系”这一类问题。先不要急着背实现：它的核心任务是：判断两棵节点标签无关的树是否具有完全相同的结构。
...
寻找树中心并递归排序子树编码，生成无根树规范形式。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **AHU 树同构** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义节点、孩子和递归函数的返回值
2. 处理空节点或叶子节点基例。在本算法中，对应演示动作是：寻找树中心并递归排序子树编码，生成无根树规范形式
3. 递归取得各子树结果
4. 在当前节点合并并返回
...
3. 对照状态栏观察“寻找树中心并递归排序子树编码，生成无根树规范形式”是否正在发生，并解释为什么没有选择其他候选。
...
- 不看代码，你能否用一句话说清 **AHU 树同构** 在每一步维护的状态？
