...
**汉诺塔 Tower of Hanoi** 属于“选择、递归与撤销”这一类问题。先不要急着背实现：它的核心任务是：递归分解圆盘搬运，展示全部 2ⁿ-1 次合法移动。枚举决策树中的可行方案，在前缀已经不可能成功时立即剪枝并撤销状态。
...
递归分解圆盘搬运，展示全部 2ⁿ-1 次合法移动。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
2. 遍历当前可选决策。在本算法中，对应演示动作是：递归分解圆盘搬运，展示全部 2ⁿ-1 次合法移动
3. 做选择并同步约束状态
4. 递归后撤销所有本层修改
