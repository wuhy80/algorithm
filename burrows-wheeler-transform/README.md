...
**Burrows-Wheeler 变换 BWT** 属于“重复、概率与可逆表示”这一类问题。先不要急着背实现：它的核心任务是：可逆地重排字符串，让相同字符聚集以提升后续压缩效率。
...
生成并排序循环旋转，提取最后一列和主索引展示字符聚集。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **Burrows-Wheeler 变换 BWT** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 分析符号频率或可重复片段
2. 选择码字、区间或字典引用。在本算法中，对应演示动作是：生成并排序循环旋转，提取最后一列和主索引展示字符聚集。
3. 按顺序输出可唯一解析的标记
4. 使用同一规则解码并逐项校验原文
...
3. 对照状态栏观察“生成并排序循环旋转，提取最后一列和主索引展示字符聚集。”是否正在发生，并解释为什么没有选择其他候选。
