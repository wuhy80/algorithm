...
**FM-index** 属于“后缀、自动机与全文索引”这一类问题。先不要急着背实现：它的核心任务是：在接近压缩文本大小的空间内完成大规模全文模式匹配。
...
基于 BWT、C 表与 Occ 计数执行反向全文检索。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **FM-index** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 确定索引表示的是前缀、后缀还是等价子串集合
2. 按字符或倍增长度增量构建结构。在本算法中，对应演示动作是：基于 BWT、C 表与 Occ 计数执行反向全文检索
3. 维护链接、排名或区间边界
4. 把查询转成路径、区间或公共前缀操作
...
3. 对照状态栏观察“基于 BWT、C 表与 Occ 计数执行反向全文检索”是否正在发生，并解释为什么没有选择其他候选。
4. 到达终态后只改一个边界条件，例如空输入、重复值、极端顺序、断开的图或最小规模，再重新运行。
5. 最后尝试不看动画，只根据伪代码复现同一过程；能独立写出状态变化才算真正掌握。

## 复杂度怎么分析

**结论：查询 O(pattern length)，空间接近压缩文本**
