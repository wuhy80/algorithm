...
**洪水填充 Flood Fill** 属于“图搜索与访问状态”这一类问题。先不要急着背实现：它的核心任务是：修改图像或网格中与起点连通且颜色相同的区域。
...
从起点向四邻域扩散并逐格替换同色连通区域。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **洪水填充 Flood Fill** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 由边构建邻接关系并确定有向性
2. 初始化起点、visited 与栈 / 队列。在本算法中，对应演示动作是：从起点向四邻域扩散并逐格替换同色连通区域
3. 取出节点并检查相邻边
4. 发现新节点时记录来源、层数或颜色
...
3. 对照状态栏观察“从起点向四邻域扩散并逐格替换同色连通区域”是否正在发生，并解释为什么没有选择其他候选。
...
- 不看代码，你能否用一句话说清 **洪水填充 Flood Fill** 在每一步维护的状态？
