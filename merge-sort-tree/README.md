...
**归并排序树 Merge Sort Tree** 属于“区间分解与可合并摘要”这一类问题。先不要急着背实现：它的核心任务是：查询任意下标区间中小于某值的数量或静态第 k 小元素。
...
线段树节点保存有序子数组，以二分回答区间顺序统计。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **归并排序树 Merge Sort Tree** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 定义区间摘要以及 combine 是否满足结合律
2. 构建标准区间或块的摘要。在本算法中，对应演示动作是：线段树节点保存有序子数组，以二分回答区间顺序统计
3. 查询时选择完全覆盖的块并合并
4. 更新时只修改受影响路径并重算祖先
...
- 先修内容：[线段树 Segment Tree](https://github.com/wuhy80/algorithm/tree/main/segment-tree/)、[归并排序 Merge Sort](https://github.com/wuhy80/algorithm/tree/main/merge-sort/)
- 直接后续：暂无强制关联项。
...
- 不看代码，你能否用一句话说清 **归并排序树 Merge Sort Tree** 在每一步维护的状态？
