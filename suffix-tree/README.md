...
**后缀树 Suffix Tree** 属于“后缀、自动机与全文索引”这一类问题。先不要急着背实现：它的核心任务是：用线性规模索引表示全部后缀，支持快速子串、重复和匹配查询。
...
把所有后缀插入压缩 Trie，在部分匹配位置切分边并共享公共前缀。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
所谓不变量，就是算法每一步开始和结束时都必须为真的事实。调试 **后缀树 Suffix Tree** 时，最有效的方法不是盯着最终答案，而是在每次单步后检查这条不变量。只要某一帧不再满足它，错误通常就在上一帧的边界更新、状态转移或数据结构维护中。

## 算法步骤

1. 确定索引表示的是前缀、后缀还是等价子串集合
2. 按字符或倍增长度增量构建结构。在本算法中，对应演示动作是：把所有后缀插入压缩 Trie，在部分匹配位置切分边并共享公共前缀。
3. 维护链接、排名或区间边界
4. 把查询转成路径、区间或公共前缀操作
...
3. 对照状态栏观察“把所有后缀插入压缩 Trie，在部分匹配位置切分边并共享公共前缀。”是否正在发生，并解释为什么没有选择其他候选。
...
- 先修内容：[后缀数组与 LCP](https://github.com/wuhy80/algorithm/tree/main/suffix-array-lcp/)、[后缀自动机 Suffix Automaton](https://github.com/wuhy80/algorithm/tree/main/suffix-automaton/)
- 直接后续：暂无强制关联项。
- 同类比较：[Aho-Corasick 多模式匹配](https://github.com/wuhy80/algorithm/tree/main/aho-corasick/)、[Booth 最小表示法](https://github.com/wuhy80/algorithm/tree/main/booth-minimum-rotation/)、[Duval / Lyndon 分解](https://github.com/wuhy80/algorithm/tree/main/duval-lyndon-factorization/)、[FM-index](https://github.com/wuhy80/algorithm/tree/main/fm-index/)

学习顺序建议是：先用先修算法理解基础状态，再比较同类算法在“前提、维护信息、复杂度、是否可恢复答案”上的差异，最后进入把本算法作为组件的后续主题。

## 自测问题

- 不看代码，你能否用一句话说清 **后缀树 Suffix Tree** 在每一步维护的状态？
