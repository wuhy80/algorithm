...
**笛卡尔树 Cartesian Tree** 属于“递归子树与层级关系”这一类问题。先不要急着背实现：它的核心任务是：把序列顺序与堆优先级结合，用于 RMQ、区间结构和后缀算法。
...
单调栈维护右脊，使中序次序等于原数组且节点同时满足最小堆序。。这句话里的动作不是界面效果，而是算法正确性的关键过程。把它拆开看，可以得到“输入约束 → 状态变化 → 不变量仍成立 → 答案范围缩小”这条主线。
...
1. 定义节点、孩子和递归函数的返回值
2. 处理空节点或叶子节点基例。在本算法中，对应演示动作是：单调栈维护右脊，使中序次序等于原数组且节点同时满足最小堆序。
3. 递归取得各子树结果
4. 在当前节点合并并返回
